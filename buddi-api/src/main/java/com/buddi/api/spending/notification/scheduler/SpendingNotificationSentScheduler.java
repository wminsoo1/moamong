...
public class SpendingNotificationSentScheduler {

    private final SpendingNotificationSentRepository repository;
    private final SpendingNotificationSentReserver reserver;
    private final SpendingNotificationSentProcessor processor;
    ...
    public void poll() {
        reserver.reserve().forEach(id -> {
            try {
                ...
                log.error("지출 알림 전송 실패 (sentId: {}): {}", id, e.getMessage());
            }
        });
    }
    ...
    public void recover() {
        ...
        log.warn("지출 알림 PROCESSING 상태 복구: {}건", stuck.size());
    }
}
